...
\begin{itemize}
    ...
    \item si introduce la possibilit\`a di rinominare i registri: ogni volta che viene fatto l'issue di un istruzioni i registri vengono rinominati con i registri di quella specifica reservation station;
\end{itemize}
...
\begin{itemize}
    ...
    \item issue: viene prelevata la prossima istruzione dalla coda, se la matching reservation station \`e libera (no structural hazards), l'istruzione viene issued alla reservation station:
        \begin{itemize}
            ...
            \item altrimenti, con gli indentificatori delle reservation stations che produrranno l'operando (e.g. le RS dell'unit\`a di add aspettano un operando che sar\`a prodotto dall'unit\`a di divisione);
        \end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{architettura-tomasulo.png}
    \caption{Architettura Tomasulo}
    \label{fig:architettura-tomasulo}
\end{figure}
Nelle reservation station sono presenti dei dati:
\begin{itemize}
    \item op: operazione che deve essere fatta;
    \item $v_j, v_k$: valori degli operandi;
    \item $Q_i, Q_j$: RS che produrranno l'operando;
    \item A: valori immediati;
    \item Busy: status della reservation station;
\end{itemize}
Inoltre \`e presente anche il \textbf{register status} che \`e una parte di memoria dove sono contenuti i valori di output delle operazioni associate a ogni registro.

Quando le reservation station per una unit\`a sono esaurite c'\`e bisogno di uno stallo strutturale.
...
\newpage
\section{Speculazione}
La speculazione \`e tecnica basata sull'hardware che permette ridurre gli effetti delle dipendenze di controllo in sistemi che sfruttano la schedulazione dinamica. Se un processore supporta il branch prediction con schedulazione dinamica viene fatto l'issue delle istruzioni come se il branch fosse preso, con la \textbf{speculazione queste istruzioni vengono anche eseguite}. Per implementare la speculazione si parte dall'architettura di Tomasulo (\ref{fig:architettura-tomasulo}) e la si estende:
\begin{itemize}
    \item l'update dei registri e della memoria \`e fatto solo durante la fase di commit che avviene in modalit\`a \textit{in-order};
    \item \textbf{ReOrder Buffer (ROB)}: \`e una struttura dati che contiene i risultati delle istruzioni non committate, oltre a questo contiene dei registri virtuali che si vanno a integrare con i buffer dell'architettura di Tomasulo;
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{speculation-architecture.png}
    \caption{Speculation Architecture}
    \label{fig:speculation-architecture}
\end{figure}
La fasi di esecuzione sono:
\begin{itemize}
    \item issue: un'istruzione \`e presa dalla coda se una reservation station \`e libera e uno slot nel reorder buffer \`e libero;
\end{itemize}
Per gestire le eccezioni in modo preciso, vengono messe insieme alle istruzioni nel reorder buffer, solo quando verr\`a fatto il commit l'eccezione sar\`a eseguita. 

Gli stalli causati da WAW e WAR vengono eliminati del register renaming, mentre gli stalli causati da RAW continuano a causare problemi.

Un altra struttura dati \`e il \textbf{Register Alias Table (RAT)} che specifica se un certo dato va preso delle resrvation station o dal reorder buffer.

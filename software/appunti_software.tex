...
\subsection{Glossary}
The \textbf{glossary} or \textbf{conceptual diagram} with \textbf{UML class diagram}, is a way to describe the objects of our system processing. Thanks to UML (\emph{Unified Modeling Language}) it's possible the relationships between the various objects. UML has many characteristics, some of which are:
\begin{itemize}
  \item The Object is the \textbf{Model} of an item.
...
When links are formed between classes there can multiplicity of the nubmer of classes dependent:
\begin{itemize}
  \item \texttt{1}
  \item \texttt{0..1}
  \item \texttt{0..n}
...
\textbf{Composition} is indicated with a \emph{diamond} and there are two kinds:
\begin{itemize}
  \item when the diamond \textbf{is hollow}, where \emph{the class A which point to B}, means that \emph{A} is part of \emph{B}.
  \item when the diamond \textbf{is full}, the lifecycle are all associated, if a component dies all the other component die.
\end{itemize}
...
\begin{example}{University}{university}
...
The \textbf{deployment diagram} is a way to represent how the system is deployed on the physical hardware.

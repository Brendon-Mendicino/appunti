...
\subsection{Semafori}
...
Inizialmente i semafori venogno implementati con \emph{busy waiting},
...
Esistono dei casi in cui (\textbf{wait on codition}) la struttura va guradata al di fuori di P(), per questo motivo severno dei lock per ogni variabile all'interno delle
\begin{lstlisting}[language=c]
...
\end{lstlisting}
In questo caso se \texttt{use\_a\_thing} arriva per primo al lock ci sar\`a un deadlock.

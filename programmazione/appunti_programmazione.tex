...
\begin{itemize}
  ...
  \item \textbf{Enhanced Second-Chance Algorithm}: si utilizza una combinazione di reference e modify per scegliere il frame:
    \begin{itemize}
      \item (0, 0): pagina miglire
      \item (0, 1): la pagina \`e dirty, quindi va scritta su disco, si va avanti
      \item (1, 0): pagina refernziata, si setta il ref a 0 zero e si va avanti
      \item (1, 1): pagina dirty e referenziata, molto probabile che sia utilizzata di nuovo
    \end{itemize}
\end{itemize}
Esistono gli algoritmi basati sul conteggio, \textbf{Counting Algorithms}:
\begin{itemize}
  \item \textbf{Least Frequently Used (LFU):} si prende come pagina quella con il minor contatore
  \item \textbf{Most Frequently Used (MFU):} si prende la pagina con il numero pi\`u questo argomento si basa sul fatto che una pagina appena rimpiazzata ha il contatore basso
\end{itemize}

Fino ad ora abbiamo cercato di minimizzare il numero di page fault, adesso cerechiamo dei metodi per migliorare ancora le prestazioni. Un'altra tecnica importante \`e il '\textbf{Page-Buffering Algorithm}, dove si mantiene una lista di frame liberi, in questo modo dopo un fault si copia la pagina all'iterno del frame libero, si cerca sempre di mantenere la lista dei free frame sempre con delle entry allora si seglie una pagina vittima e la si rimpiazza superata una certa soglia, le pagine vittime devono essere scelte quanto pi\`u frequentemente possibile e soprattutto quando \`e possibile, cos\`i da avere un pool sempre ampio di frame liberi. Possibilimente viene anche mantenuta una \textbf{lista di pagine modificate}, e quando il bakcing store \`e idle, queste vengono salvate li sono settate a non-dirty. In alcuni casi l'applicazione deve poter avere sotto controllo la gestione della memoria, come ad esempio i DB, che permetti di baipassare buffering, locking, etc...

\subsection{Allocazione delle pagine}
Per gestire l'allocazione delle pagine si possono due schemi diversi, allocazion fissa, allocazione priritaria.

\textbf{Allocazione fissa}:
\begin{itemize}
...
\textbf{Global vs Local allocation}: un processo pu\`o scegliere una frame vittima solo tra i suoi anche tra quelli degli altri? \textbf{Locale} visto nell'esempio di sopra, ogni applicazione pu\`o rimpiazzare solo le sue pagine, che crea consistenza al livello del programma ma \`e inefficente a livello di memoria, infatti rischia di essere sottoutilizzata. \textbf{Globale}, esista una lista di free frame, si fa in modo che questa lista non arrivi mai a 0, l'idea di massima \`e che superata una soglia si iniziano a scegliere delle vittime, fino ad arrivare di nuovo ad un altra solgia, che superata permette il reinserimento dei frame, questa soluzione permette un throughput molto maggiore di quella a rimpiazzo locale.

Quando si usano queste politiche bisogna tenere in conto che alcune architetture sono NUMA (Non-Uniform Memory Access), vuol dire alcune di memoria sono pi\`u vicine ad un core rispetto che agli altri, questo vuol dire che se quel core accede ad altra zona di memoria i tempi saranno pi\`u lunghi, un buffer di pagine libere doverbbe tenere in considerazione questo aspetto quando venogno assegnate le pagine.
...
\subsection{Rc}
Esistono delle situazioni in cui si potrebbe volere un possesso condiviso ad un dato, questo \`e un problema perch\'e una dato pu\`o essere posseduto da un'unica variabile alla volta, questa condizione viene controllata dal borrow checker durante la compilazione, per questo motivo Rust fornisce dei tipi in grado di fare queste operazioni, che sono tipi della famiglia \texttt{Cell}, infatti in alcuni casi avere un singolo owner \`e troppo limitante. Tutte queste struct implementano il paradigma della \textbf{Interior Mutability}, ovvero \`e possibile ottere un riferimento mutabile ad un dato partendo da un rifermento semplice. Bisogna ricordare che la regola del borrow checker che, \textbf{possono esistere pi\`u rifermenti o un solo rfierimento mutabli per ogni istante di tempo} rimane sempre valido, usato l'interior mutability questi check vengono fatti a run-time (se le conidzioni falliscono il programma va in panic), causando un overhead, infatti questo pattern va usato solo quando strettamente necessario.

\subsection{Cell}
La struct \texttt{Cell<T>} peremtte di muovere dei valori dentro e fuori, non \`e possibile ottere il valore contenuto all'interno senza prima rimpiazzarlo
\begin{lstlisting}[language=rust]
struct SomeStruct {
    regular_field: u8,
...
\end{lstlisting}
il metodo \texttt{get} \`e utilizzabile solo il dato implementa \texttt{Copy}.

\subsection{RefCell}
\texttt{RefCell<T>} mantine il puntatore ad un cella di memoria che pu\`o essere condivisa, questo viene fatto mantenendo al suo interno quante reference sono attive in un determinato istante, a differenza di \texttt{Cell<T>} il valore pu\`o essere ottenuto con una mutable reference (se si cerca di chiderene una mutabile mentre altre sono attive il programma va in panic), infatti \texttt{RefCell<T>} viene spesso usato insieme a \texttt{Rc<T>}
\begin{lstlisting}[language=rust]
...
\subsection{Cow}
Esiste anche il tipo \texttt{Cow<T>} che implementa il pattern del \textbf{Copy On Write}, questo viene fatto usando \texttt{Cow} come un \texttt{enum}
\begin{lstlisting}[language=rust]
enum Cow<'a, B>
...
\end{lstlisting}
questo fa si che ogni volta che si cerca di mutare un valore e il nostro valore \`e Borrowed, viene fatta una copia di B e ci viene ritornato all'interno di Owned.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{schema-smart-pointers.png}
  \caption{Schema Smart Pointers}
  \label{fig:schema-smart-pointers}
\end{figure}
...
Prima di tutto Rust si crea delle \texttt{struct} che rappresentano le dipendenze per ogni stato della macchina, in questo caso per la figura \ref{fig:macchina-a-stati-finiti} sono
\begin{lstlisting}[language=rust]
struct S0 {
  h1: FileHandle,
  h2: FileHandle,
...
struct S3 {}
\end{lstlisting}
inlotre Rust genera anche un \texttt{enum} che racchiude tutti i possibili stati
\begin{lstlisting}[language=rust]
enum CopySM {
  s0(S0),
  s1(S1),
...
\begin{lstlisting}[language=rust]
fn copy(h1: FileHandle, h2: FileHandle) -> impl Future<Output = std::io::Result<()>> {
  CopySM::s0(
    S0 { h1, h2 }
  ) 
}
\end{lstlisting}
In modo nativo Rust non ha nessun modo di chimare il metodo \texttt{poll}, la gestione dell'esecuzione viene scelta dal programmatore scegliendo una libreria scritta appositamente, la scelta \`e qualla di associare al nostro codice un \textbf{Executor}, si possono scegliere tra tre librerie (le migliori): Tokio, smol, async-std (che fornisce una controparte asincrona per ogni funzione della libreria standard, a met\`a tra tokio e smol) (\emph{"\`E un po come l'acqua tiepida, fa schifo a tutti" .cit Malnati}).
...
int main() {
  ...
  \item
  \begin{lstlisting}[language=rust]
All'interno di un programma \`e definita la seguente struttura dati
struct Bucket {
data: Vec<i32>,
threshold: Option<i32>
}
Usando il debugger si \`e determinato che, per una istanza di Bucket, essa \`e memorizzata
...
Cosa \`e possibile dedurre relativamente ai valori contenuti dei vari campi della singola istanza?
  \end{lstlisting}
...
where R: Send {
  ...
  fn execute<F>(&self, mut f: F) -> Result<R, Box<dyn Error>>
  where F: FnMut() -> R + Send {
    {
